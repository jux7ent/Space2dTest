...
namespace Kuhpik.Pooling
{
    internal static class PoolingSystem
    {
        ...
        public static void Init(IList<IPoolData> datas, int capacity = 0)
        {
            ...
            poolsContainer = new GameObject("Pool");
        ...
        static void ExtendPool(GameObject @object)
        {
            ...
            copy.transform.parent = poolsContainer.transform;
        ...
        public static GameObject GetObject(GameObject @object, Vector3 position)
        {
        ...
        public static void TryPoolAllByPrefab(GameObject prefabObj) {
            if (!pooledObjects.ContainsKey(prefabObj)) {
                Debug.LogWarning($"There is no prefab in pool. Name: {prefabObj.name}");
                return;
            }
            
            List<GameObject> allPooledObjectsFromThisPrefab = pooledObjects[prefabObj];
            for (int i = 0; i < allPooledObjectsFromThisPrefab.Count; ++i) {
                if (allPooledObjectsFromThisPrefab[i] != null) {
                    Pool(allPooledObjectsFromThisPrefab[i]);
                }
            }
            
            pooledObjects[prefabObj].Clear();
        }
        ...
        static ObjectData GetData(GameObject @object)
        {
            ...
            if (!pooledObjects.ContainsKey(@object)) {
                pooledObjects.Add(@object, new List<GameObject>());
            }
            pooledObjects[@object].Add(data.gameObject);
