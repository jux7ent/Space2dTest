...
public class ExtentedTransformEditor : Editor {
    ...
    private void StandardTransformInspector() {
        ...
        Vector3 initialGlobalPosition = _transform.position;

        EditorGUI.BeginChangeCheck();
        Vector3 localPosition = EditorGUILayout.Vector3Field("Local Position", _transform.localPosition);
        ...
        EditorGUI.BeginChangeCheck();
        Vector3 globalPosition = EditorGUILayout.Vector3Field("Global Position", _transform.position);
        if (EditorGUI.EndChangeCheck())
            didGPositionChange = true;

        // Apply changes with record undo
        if (didPositionChange || didRotationChange || didScaleChange || didGPositionChange) {
            ...
            if (didGPositionChange)
                _transform.position = globalPosition;
        }
        ...
        if (selectedTransforms.Length > 1) {
            foreach (var item in selectedTransforms) {
                ...
                if (didGPositionChange) {
                    item.position = ApplyChangesOnly(
                        item.position, initialGlobalPosition, _transform.position);
                }
            }
        }
    }
